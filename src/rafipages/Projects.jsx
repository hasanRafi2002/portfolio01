...
const Projects = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] grid-bg p-6 flex items-center justify-center">
      <div className="max-w-6xl mx-auto w-full">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="text-center mb-12">
          ...
          <h1 className="text-3xl md:text-4xl font-display font-bold text-[var(--text-primary)] text-glow mb-3">My Projects</h1>
          <p className="text-base md:text-lg text-[var(--text-secondary)]">I'm currently focused on iOS development — here's that work, plus earlier web projects I built along the way.</p>
        </motion.div>
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:gap-10">
          {projectCategories.map((category, i) => (
            <motion.div key={category.id} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} transition={{ delay: i * 0.1 }}>
              <Card className="flex flex-col justify-between h-full p-6 rounded-xl hover:-translate-y-1">
                <CardHeader className="flex flex-col items-center gap-4 mb-4">
                  ...
                  <CardTitle className="text-xl font-display font-bold text-center text-[var(--text-primary)]">{category.title}</CardTitle>
                </CardHeader>
                <CardContent className="text-center text-[var(--text-secondary)] mb-6 text-sm md:text-base leading-relaxed">{category.description}</CardContent>
