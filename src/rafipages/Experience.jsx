    ...
    <div className="min-h-screen bg-[var(--bg-primary)] grid-bg">
      <div className="container max-w-6xl px-4 py-20 mx-auto">
        <div className="mb-16 text-center">
          ...
          <h2 className="mb-4 text-4xl font-display font-bold text-[var(--text-primary)] text-glow">Work Experience</h2>
        ...
        <div className="relative space-y-8">
          ...
          {experienceData.map((exp, i) => (
            <motion.div key={exp.id} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} transition={{ delay: i * 0.1 }} className="relative pl-16">
              ...
              <div className="glass glass-hover p-6 rounded-xl hud-corners relative">
                <div className="space-y-4">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <h4 className="flex items-center gap-2 text-xl font-display font-semibold text-[var(--text-primary)]"><FaBriefcase className="text-omni" /> {exp.role}</h4>
                      <p className="flex items-center gap-2 mt-2 text-[var(--text-secondary)]"><FaBuilding className="text-omni/70" /> {exp.company}</p>
                    </div>
                    <p className="flex items-center gap-2 text-sm text-omni/70 font-mono"><FaCalendarAlt className="text-omni/70" /> {exp.duration}</p>
                  </div>

                  <p className="leading-relaxed text-[var(--text-secondary)]">{exp.description}</p>
